...
{
    FILE_LOG(logINFO) << "CircularQueue::1";
    pData = (char*)malloc(len_);
    //FILE_LOG(logINFO) << "CircularQueue::2";
    //for (std::size_t i = 0; i != len_; ++i)
    //    pData[i] = '?';
    FILE_LOG(logINFO) << "CircularQueue::3";
}
...
{
    stop();
    FILE_LOG(logINFO) << "written = " << written << ", lost = " << lost << ", read = " << read << ", logged = " << written + lost;
}
